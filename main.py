...
create_dir = os.getcwd() + '\\' + 'createimages'
d = datetime.now()
today = datetime.today()
date_p = f'{today.strftime(f"%Y-%m-%d_%H;%M_")}'

btn_rep = InlineKeyboardButton('Да, переслать', callback_data='rep')
inline_kb = InlineKeyboardMarkup().add(btn_rep)


def photo_name(date_p, user_id):
    return date_p + str(user_id) + '.jpg'
...
def write_text(path_image):
    ...
    myimage.save(create_dir + '\\' + path_image.split('\\')[-1])
...
async def handle_docs_photo(message):
    path_image = f'{images_dir}\\{photo_name(date_p, message.from_user.id)}'
